...
const getClubId = async (req, res) => {
  try {
    ...
    for (const club of allClubs) {
      if (
        club.assignedConvenor &&
        club.assignedConvenor.toString() === req.student._id.toString()
      ) {
        console.log(
          club.assignedConvenor != undefined &&
        ...
        return res.status(200).json({
          success: true,
          error_code: 200,
          message: "Club id fetched",
          data: { clubId: club._id }, // corrected to clubId
        });
      }
    }
